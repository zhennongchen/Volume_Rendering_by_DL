%% Description
% this script will generate rotated + (boxed) image and segmentation data
% for each case (all time frames)
%% add function path
clear all;
...
for num = 1:size(class_list,1)
    ...
    % define save folders
    savefolder = [main_path,'mat_data/',info.patient_class,'/',info.patient];
    mkdir(savefolder);
    imagesavefolder = [savefolder,'/image_rotate_box'];
    mkdir(imagesavefolder);
    segsavefolder = [savefolder,'/seg_rotate_box'];
    mkdir(segsavefolder);
    if isfile([imagesavefolder,'/img_0.mat']) == 1 && isfile([segsavefolder,'/pred_s_0.mat']) == 1
        disp('already done')
        continue;
    end
