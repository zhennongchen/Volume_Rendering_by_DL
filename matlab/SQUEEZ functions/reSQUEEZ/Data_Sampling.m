...
for j = info.timeframes
    ...
    while (dummy - z_lim(2)) > info.rawdata_slicethickness
        ...
        if isempty(info.lvot_bottom)
            ...
            if dummy < Mesh(j).rotated_verts(info.lvot_bottom,3) && b
               % decresing dummy finally lower than the lvot_bottom
                info.lvot_limit(j) = c;
                b = false;
            end
        end    
    ...
    for k = 1:numel(slice) %Slice loop
        ...
        % assign RSct to each angle in one slice
        c = 1; dummy = 0;
        while 2*pi - dummy > bin_width %Azimuthal loop
            ...
            if nnz(list) == 0
                ...
                data_err{k}(1,c) = NaN; % 1st row is error values
                data_err{k}(2,c) = mean([(c-1)*bin_width, c*bin_width]); % 2nd row is corresponding theta value
            else
                [~,locb] = ismember(slice{k}(ind,:),Mesh(j).rotated_verts,'rows');
                data{k}(1,c) = Mesh(j).RSct_vertex(locb);
                data{k}(2,c) = thetas(ind);
                data_err{k}(1,c) = Mesh(j).Corr_Err(locb)*info.desired_res;
                data_err{k}(2,c) = thetas(ind);
            end    
            c = c + 1;
            dummy = c*bin_width;
        end    
    ...
    Mesh(j).Polar_Data = data; % it saves the RSct
    Mesh(j).Polar_Data_err = data_err;
