...
%%
% WMA_region_size_counting_0.15_0.1: yellow: RSct>-0.15, red: RSct>-0.10
...
for num = 1:size(class_list,1)
    ...
    save_folder_dir = [data_path,'Volume_Rendering_Movies_MATLAB/',patient_class,'/',patient_id];
    save_folder = [save_folder_dir,'/WMA_measures_', num2str(lower_bound),'_', num2str(upper_bound)];
    
    region_measure_done = isfile([save_folder,'/WMA_region_size_counting_',num2str(lower_bound),'_',num2str(upper_bound),'.txt']);
    movie_done = isfile([save_folder_dir,'/SQUEEZ_movies/',patient_id,'_AllViews_4DSqueez.mp4']);
    ...
    mkdir(save_folder_dir);mkdir(save_folder);
    ...
    if movie_done == 0
        % make SQUEEZ movie
        info.save_movie_path = [save_folder_dir,'/SQUEEZ_movies/'];
    ...
    threshold_RSct_moderate = lower_bound;
    threshold_RSct_severe = upper_bound;
    ...
    view_angle = [0:60:350];
    image_save_folder = [save_folder,'/WMA_labeled_pngs_',num2str(lower_bound),'_',num2str(upper_bound)];
    ...
    save_file_basename = ['WMA_regions_measures_per_angle_', num2str(lower_bound), '_', num2str(upper_bound), '.mat'];
    [WMA_patches_measures] = Measure_Region_of_WMA_Size_From_Labeled_Volshow(WMA_exist, seg_WMA_multi,patient_id,image_save_folder,view_angle,rows_of_Mitral_Valve,rows_of_Mitral_Valve_center, dilate_size,multi_class_in_WMA_measure,config_label_MI,save_file_basename);
   
    % save as txt file
    fid = fopen([save_folder,'/WMA_region_size_counting_', num2str(lower_bound), '_', num2str(upper_bound), '.txt'],'wt');
